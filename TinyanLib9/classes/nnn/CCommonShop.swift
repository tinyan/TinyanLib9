...
public class CCommonShop : CCommonGeneral , SKProductsRequestDelegate , SKPaymentTransactionObserver
{
	...
	func getProductIDSet(objects:[AnyObject]) -> Set<String>
	{
		return Set(objects as! [String])
	}
	...
	{
		let productID = getProductIDSet(objects)
	...
	func startPayment(object:String)
	{
		...
		let productID = getProductIDSet([object])
		let request  = SKProductsRequest(productIdentifiers: productID)
		request.delegate = self
		startIndicator()
		request.start()
		...
		*/
		...
		if m_shopMode == SHOP_GETPRICE_MODE
		{
			...
			for product in response.products
			{
				...
				let str = formatter.stringFromNumber(product.price)
				let ids = (product as SKProduct).productIdentifier
...
//	}
	...
	override public init(modeNumber:Int , game : CCommonGame, size:CGSize)
	{
		...
		if let json = CCommonJsonObject.loadByFilename("init/shop")
		{
			getCommonParam(json)
			...
			if let productList : [String] = json.getArrayObject(keyList: "productList")
			{
				m_productIDList = productList
				//var cnt = m_productIDList.count
				m_priceList  = []
				m_priceListGetFlag = []
				for _ in 0 ..< m_productIDList.count
				{
			...
			let top = "common"
			if var _: AnyObject = json.getAnyObject(top)
			{
		...
		let shopFont = CCommonSpriteCutter(filename: m_shopItemDescFontFilename, x: 8, y: 8)
		
		for i in 0 ..< m_shopItemDesc.count
		{
			let mes = CCommonMessage64(size: m_shopItemDescFontSize, font: shopFont)
			m_shopItemDescMessage.append(mes);
			
			let x : CGFloat = m_shopItemDescPoint.x + CGFloat(i) * m_shopItemDescNext.dx
			let y : CGFloat = m_shopItemDescPoint.y + CGFloat(i) * m_shopItemDescNext.dy
	...
	override public func EnterMode()
	{
		...
		{
			let x : CGFloat = m_shopItemDescPoint.x + CGFloat(i) * m_shopItemDescNext.dx
			let y : CGFloat = m_shopItemDescPoint.x + CGFloat(i) * m_shopItemDescNext.dy
	...
	public func startShop()
	{
		...
		_ = m_game as CCommonGame
		
		if !m_shopButtonCreateFlag
		{
			for i in 0 ..< m_productIDList.count
			{
				let itemPoint = getItemBasePoint(i)
				let itemTexture = m_shopItemPic.cutSprite(i)
				let itemSprite = SKSpriteNode(texture: itemTexture,color : UIColor.clearColor(),size:m_itemSize)
				itemSprite.position = itemPoint
				self.addChild(itemSprite)
				
				//already buy
				let alreadyBuySprite = SKSpriteNode(texture:m_alreadyBuyPic,color:UIColor.clearColor(),size:m_alreadyBuySize)
				itemSprite.addChild(alreadyBuySprite)
				m_alreadyBuyTexture.append(alreadyBuySprite)
				
				
				//button
				let buyTexture = m_itemButtonPic.cutSprite(0)
				let buyButton = CCommonButton(general: self, texture: buyTexture, size: m_buyButtonSize)
				buyButton.setNumber(3 + i * 2 + 0)
				buyButton.position = getBuyButtonPoint(i)
				
				let label = SKLabelNode(text: m_priceList[i])
				...
				let restoreTexture = m_itemButtonPic.cutSprite(1)
				let restoreButton = CCommonButton(general: self, texture: restoreTexture, size: m_restoreButtonSize)
				restoreButton.position = getRestoreButtonPoint(i)
				restoreButton.setNumber(3 + i * 2 + 1)
				
				let label2 = SKLabelNode(text: m_restoreText)
