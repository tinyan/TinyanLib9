...
//	}
	...
	//	println("otherapp::init")
		
//		if var json = CCommonJsonObject.loadByFilename("init/config")
		if let json = CCommonJsonObject.loadByFilename("init/otherapp")
		{
		//	println("otherapp::found jsonfile")
			
			getCommonParam(json)
			m_commonMenu = CCommonMenu(general: self, json: json, menu: "menu")
			self.addChild(m_commonMenu)
			
			if var _ : AnyObject = json.getAnyObject("common")
			{
				let top = "common"
	...
	public override func onUpdate(currentTime: CFTimeInterval)
	{
		...
		if let cmd = calcuCommonButtonResult()
		{
			...
			else if cmd != -1
			{
				...
				if app >= 0 && app < m_appName.count
				{
					...
					if let url = NSURL(string:m_link[app])
					{
...
//					var groupAction = SKAction.group([turnAction,fadeoutAction,scaleAction])
					...
					let button = menu.m_button[number]
					let sound = button.m_sound
...
//					button.runAction(groupAction)
			...
			if number >= 3
			{
				...
				let button = m_appButton[number-3]
				let sound = button.m_sound
				m_game.playSound(sound)
	...
	public func createAppButton()
	{
		...
		for i in 0 ..< m_appName.count
		{
			let texture = SKTexture(imageNamed: m_picFilename[i])
			let button = CCommonButton(general: self, texture: texture, size: m_size[i])
			button.position = m_point[i]
			button.setNumber(i+3)
			button.zPosition = COMMON_BUTTON_LAYER_Z
			
			let label = SKLabelNode(text: m_appName[i])
			...
			{
				let ssSprite = SKSpriteNode(imageNamed: m_ssPicFilename[i])
			...
			if commentList.count > 0
			{
				for k in 0 ..< commentList.count
				{
					...
					let commentLabel = SKLabelNode(text:commentList[k])
