...
public class CCommonGame
{
	...
	public init(viewController:CCommonViewController)
	{
		...
		{
			let tmp = m_deviceScreenSize.width;
		...
		if let json = CCommonJsonObject.loadByFilename("init/game")
		{
			if let highscoreName : [String] = json.getArrayObject(keyList: "highscoreName")
			{
				m_highscoreName = highscoreName
			}
			if let achievementName : [String] = json.getArrayObject(keyList: "achievementName")
			{
				m_achievementName = achievementName
			}
			if let gameScreenSize : CGSize = json.getObject(keyList:"gameScreenSize")
			{
				m_gameScreenSize = gameScreenSize
			}
			if let showsFPS : Bool = json.getObject(keyList:"showsFPS")
			{
				m_showsFPS = showsFPS
			}
			if let showsNodeCount : Bool = json.getObject(keyList:"showsNodeCount")
			{
	...
	public func createCommonHighscoreAndAchievement(highscoreNameList highscoreNameList:[String] , achievementNameList:[String])
	{
	...
	public func setupDefaultConfig()
	{
		...
		let soundFlag = m_userConfigData.getBoolData(COMMON_CONFIG_SOUND_FLAG)
