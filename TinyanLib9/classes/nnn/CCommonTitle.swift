...
//	}
	...
	override public init(modeNumber:Int , game : CCommonGame, size:CGSize)
	{
		...
		if let json = CCommonJsonObject.loadByFilename("init/title")
		{
			getCommonParam(json)
		...
		if m_commonCommand == -1
		{
			if let menu = m_commonMenu
			{
				if number < menu.m_button.count
				{
					...
					let tm = 1.0
					let turnAction = SKAction.rotateByAngle(3.14, duration: tm)
					let fadeoutAction = SKAction.fadeOutWithDuration(tm)
					let scaleAction = SKAction.scaleTo(3.0, duration: tm)
					let groupAction = SKAction.group([turnAction,fadeoutAction,scaleAction])
	...
	{
		let flag = m_game.checkGamecenterEnable()
		if let menu = m_commonMenu
		{
			if menu.m_button.count > TITLE_GAMECENTER_BUTTON
			{
				let button = menu.m_button[TITLE_GAMECENTER_BUTTON]
