...
public class OAL3DVector
{
	...
	{
		let dx = vector.m_vec[0] - m_vec[0]
		let dy = vector.m_vec[1] - m_vec[1]
		let dz = vector.m_vec[2] - m_vec[2]
	...
	{
		let x = m_vec[0] * cos(radian) - m_vec[1] * sin(radian);
		let y = m_vec[0] * sin(radian) + m_vec[1] * cos(radian);
		m_vec[0]		= x;
		m_vec[1]		= y;
	}
