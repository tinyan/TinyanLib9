...
public class CCommonMessage64 : SKSpriteNode
{
	...
	public func print(point point:CGPoint , mes:String , center:Bool = false)
	{
...
//			var ln = mes.utf16Count
			let ln = mes.characters.count
//			var ln = countElements(mes)
			let delta = CGFloat(ln-1) * 0.5
...
//			y -= 0.5 * m_fontSize.height
		...
		for c in mes.utf8
		{
			...
			let nx = d % 8
			let ny = d / 8
			let texture = m_font.cutSprite(x: nx, y: ny)
			let spr = SKSpriteNode(texture: texture, size: m_fontSize)
