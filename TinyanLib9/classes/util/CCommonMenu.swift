...
public class CCommonMenu : SKSpriteNode
{
	...
	//	if var list: AnyObject = json.getAnyObject(keyList: menu,"buttonList")
		if let list : [String] = json.getArrayObject(keyList: menu,"buttonList")
		{
...
//		if var start : CGPoint = json.getCGPointObject(keyList: menu,"start")
		if let start : CGPoint = json.getObject(keyList: menu,"start")
		{
			m_buttonStart = start
		}
	//	println("bs2:\(m_buttonStart)")
		
		if let next : CGVector = json.getObject(keyList: menu,"next")
		{
			m_buttonNext = next
		}
		
		if let size : CGSize = json.getObject(keyList: menu,"size")
		{
			m_buttonSize = size
		}
		
		if let filename : String = json.getObject(keyList: menu,"filename")
		{
			m_buttonFilename = filename
		}
		
		if let sound : Int = json.getObject(keyList: menu,"sound")
		{
			m_commonButtonSound = sound
		}
		
		if let serialStart : Int = json.getObject(keyList:menu,"serial")
		{
		...
		{
			let setup = CCommonButtonSetup(json: json, buttonName: m_buttonNameList[i])
...
//		userInteractionEnabled = true
	...
	public func createAllButton(status:[Int])
	{
		...
		for i in 0 ..< m_buttonNumber
		{
			...
			if let point = setup.m_point
			{
				buttonPoint = point
			}
		//	println("bp2:\(buttonPoint)")
			
			if let sz = setup.m_size
			{
				buttonSize = sz
			}
			
			if let snd = setup.m_sound
			{
				sound = snd
			}

			if let btnN = setup.m_number
			{
				buttonN = btnN
			}
			...
			{
				//println("a")
				if let filename = setup.m_filename
				{
					//println("b")
			...
			let button = CCommonButton(general: m_parent, texture: texture, size: buttonSize)
